...
POST Register User
    ...
    ${response}=    POST On Session      registersession  /Account/v1/User        data=${requestbody}       headers=${header}
    ...
    ${auth_response}=    POST On Session    registersession    /Account/v1/Authorized    data=${requestbody}       headers=${header}
    Log To Console    ${auth_response.content}
    Should Be Equal As Integers    ${auth_response.status_code}    200

    ${token_response}=    POST On Session    registersession    /Account/v1/GenerateToken    data=${requestbody}       headers=${header}
