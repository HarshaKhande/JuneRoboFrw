...
Post register user
    ...
    ${requestbody}=    Create Dictionary    userName=${user}    password=${pass}    # &{dict} =	Create Dictionary	key=value	foo=bar
    ${header}=    Create Dictionary      Content-Type=application/json;charset=utf-8    Accept=*/*    #  mandatory header for POST request
     ${response} = Put On Session  registersession /Account/v1/User    data= ${requestbody}    headers=${header}
